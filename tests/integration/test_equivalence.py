...
from config import create_ground_truth_config, create_ground_truth_model, create_ground_truth_base_model
...
class TestNumericalEquivalenceStandardMode:
    ...
    def test_forward_pass_equivalence(self):
        ...
        input_ids = torch.randint(0, self.config_params["vocab_size"], (self.batch_size, self.seq_len))
        ...
        assert torch.allclose(new_output.logits, orig_output.logits, atol=1e-6, rtol=1e-5), \
            f"Logits differ: max diff = {(new_output.logits - orig_output.logits).abs().max()}"
        
        # Check hidden states if available
        if new_output.hidden_states is not None and orig_output.hidden_states is not None:
            for i, (new_h, orig_h) in enumerate(zip(new_output.hidden_states, orig_output.hidden_states)):
    ...
    def test_parameter_count_equivalence(self):
        ...
        assert new_params == orig_params, f"Parameter count differs: new={new_params}, orig={orig_params}"
    ...
    def test_gradient_equivalence(self):
        ...
        for name, param in self.orig_model.named_parameters():
            if param.grad is not None and name in new_grads:
                assert torch.allclose(new_grads[name], param.grad, atol=1e-5, rtol=1e-4), \
...
class TestNumericalEquivalenceParScaleMode:
    ...
    def test_forward_pass_equivalence_parscale(self):
        ...
        # Check logits are identical
        assert torch.allclose(new_output.logits, orig_output.logits, atol=1e-5, rtol=1e-4), \
    ...
    def test_cache_behavior_equivalence(self):
        ...
        assert len(new_output.past_key_values.key_cache) == len(orig_output.past_key_values.key_cache)
        assert len(new_output.past_key_values.value_cache) == len(orig_output.past_key_values.value_cache)
        
        # Cache contents should be identical
        for i, (new_k, orig_k) in enumerate(zip(new_output.past_key_values.key_cache, 
        ...
        for i, (new_v, orig_v) in enumerate(zip(new_output.past_key_values.value_cache,
    ...
    def test_aggregation_equivalence(self):
        ...
        new_hook = self.new_model.model.register_forward_hook(capture_before_aggregation)
        orig_hook = self.orig_model.model.register_forward_hook(capture_before_aggregation_orig)
...
class TestCrossAttentionNonInterference:
    ...
    def test_cross_attn_disabled_equals_base(self):
        ...
        assert torch.allclose(base_output.logits, cross_output.logits, atol=1e-6, rtol=1e-5)
