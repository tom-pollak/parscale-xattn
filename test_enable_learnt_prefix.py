...
def test_disable_learnt_prefix():
    ...
    for layer in model_with_prefix.model.layers:
        assert hasattr(layer.self_attn, "prefix_k"), (
            "prefix_k should exist when parscale_n_tokens > 0"
        )
        assert hasattr(layer.self_attn, "prefix_v"), (
            "prefix_v should exist when parscale_n_tokens > 0"
        )
        assert layer.self_attn.prefix_k.shape[2] == 8, (
            f"prefix_k should have 8 tokens, got {layer.self_attn.prefix_k.shape[2]}"
        )
    ...
    for layer in model_no_prefix.model.layers:
        assert not hasattr(layer.self_attn, "prefix_k"), (
            "prefix_k should not exist when parscale_n_tokens=0"
        )
        assert not hasattr(layer.self_attn, "prefix_v"), (
            "prefix_v should not exist when parscale_n_tokens=0"
        )
    ...
    with torch.no_grad():
        ...
        print(
            f"✓ Forward pass successful with prefix enabled, output shape: {output_with_prefix.logits.shape}"
        )
    ...
    with torch.no_grad():
        ...
        print(
            f"✓ Forward pass successful with prefix disabled, output shape: {output_no_prefix.logits.shape}"
        )
